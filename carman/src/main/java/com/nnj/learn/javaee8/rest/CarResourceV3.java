...
public class CarResourceV3 {
	...
	@GET
	public CompletionStage<Response> getCars(@QueryParam("attr") @DefaultValue("") final String filterByAttr,
	...
	public void createCar(@NotNull final JsonObject jsonSpec, @Suspended AsyncResponse response) {
		final String tmpCarUrl = uriInfo.getBaseUriBuilder().path(CarResourceV3.class).path(CarResourceV3.class, "getCar")
	...
	Response createCar(JsonObject jsonSpec, final String tmpCarUrl) {
		...
		if (strEngineType != null) {
			...
			return Response.status(Response.Status.CREATED).header("location", newCarURL).entity(car).build();
		}
